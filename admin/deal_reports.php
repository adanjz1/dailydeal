	...
	$results_per_page = 10;
		...
		if (isset($_POST['action']) && $_POST['action'] == "delete")
		{
			...
			if (count($ids_arr) > 0)
			{
				...
				header("Location: deal_reports.php?msg=deleted");
				exit();
			}
		}
		...
		$query = "SELECT reports.*, DATE_FORMAT(reports.added, '%e %b %Y %h:%i %p') AS date_added, items.* FROM abbijan_reports reports LEFT JOIN abbijan_items items ON items.item_id=reports.item_id WHERE reports.item_id<>'0' ORDER BY reports.added DESC LIMIT $from, $results_per_page";

		$result = smart_mysql_query($query);
		$total_on_page = mysql_num_rows($result);

		$query2 = "SELECT * FROM abbijan_reports WHERE item_id<>'0'";
        ...
        <?php if ($total > 0) { ?>
			...
			<?php while ($row = mysql_fetch_array($result)) { $cc++; ?>				  
				  <tr class="<?php if (($cc%2) == 0) echo "even"; else echo "odd"; ?>">
					...
					<td nowrap="nowrap" align="center" valign="middle"><a href="deal_details.php?id=<?php echo $row['item_id']; ?>&pn=<?php echo $page; ?>&column=<?php echo $_GET['column']; ?>&order=<?php echo $_GET['order']; ?>"><?php echo $row['item_id']; ?></a></td>
					...
					<td nowrap="nowrap" align="center" valign="middle">
						...
						<a href="#" onclick="if (confirm('Are you sure you really want to delete this report') )location.href='report_delete.php?id=<?php echo $row['report_id']; ?>&column=<?php echo $_GET['column']; ?>&order=<?php echo $_GET['order']; ?>&pn=<?php echo $page?>&type=deals'" title="Delete"><img src="images/delete.png" border="0" alt="Delete" /></a>
					</td>
				  </tr>
			<?php } ?>
				...
				<td colspan="6" align="left">
				  ...
				  <td align="center" colspan="8">
					<?php echo ShowPagination("reports",$results_per_page,"deal_reports.php?column=$rrorder&order=$rorder&","WHERE item_id<>'0'"); ?>
				  </td>
				  </tr>
            </table>
			</form>
