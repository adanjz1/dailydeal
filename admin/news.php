...
\*******************************************************************/
	...
	$results_per_page = 20;
		...
		$query = "SELECT *, DATE_FORMAT(added, '%e %M %Y %h:%i %p') AS news_date FROM abbijan_news ORDER BY $rrorder $rorder LIMIT $from, $results_per_page";
	
		$total_result = smart_mysql_query("SELECT * FROM abbijan_news ORDER BY news_title ASC");
			...
			<tr>
             ...
             <?php while ($row = mysql_fetch_array($result)) { $cc++; ?>
				  <tr class="<?php if (($cc%2) == 0) echo "even"; else echo "odd"; ?>">
					<td align="center" valign="middle"><input type="checkbox" class="checkboxx" name="id_arr[<?php echo $row['news_id']; ?>]" id="id_arr[<?php echo $row['news_id']; ?>]" value="<?php echo $row['news_id']; ?>" /></td>
