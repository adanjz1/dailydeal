...
\*******************************************************************/
	...
	if (isset($_GET['show']) && is_numeric($_GET['show']) && $_GET['show'] > 0)
		...
		$query = "SELECT m.*, DATE_FORMAT(m.created, '%e %b %Y %h:%i %p') AS message_date, u.fname, u.lname FROM abbijan_messages m LEFT JOIN abbijan_users u ON m.user_id=u.user_id WHERE m.is_admin='0' ORDER BY $rrorder $rorder LIMIT $from, $results_per_page";

		$result = smart_mysql_query($query);
		$total_on_page = mysql_num_rows($result);

		$query2 = "SELECT * FROM abbijan_messages WHERE is_admin='0'";
			...
			<tr>
             ...
             <?php while ($row = mysql_fetch_array($result)) { $cc++; ?>
				  <tr class="<?php if (($cc%2) == 0) echo "even"; else echo "odd"; ?>">
					<td align="center" valign="middle"><input type="checkbox" class="checkboxx" name="id_arr[<?php echo $row['message_id']; ?>]" id="id_arr[<?php echo $row['message_id']; ?>]" value="<?php echo $row['message_id']; ?>" /></td>
					...
					<td align="center" valign="middle"><span class="replies_total"><?php echo GetRepliesNum($row['message_id']); ?></span></td>
					<td nowrap="nowrap" align="center" valign="middle">
						...
						<a href="message_reply.php?id=<?php echo $row['message_id']; ?>&pn=<?php echo $page; ?>&column=<?php echo $_GET['column']; ?>&order=<?php echo $_GET['order']; ?>" title="Reply"><img src="images/reply.png" border="0" alt="Reply" /></a>
